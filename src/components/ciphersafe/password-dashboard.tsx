...
export default function PasswordDashboard({ onLock, rawMasterPassword, onLogout }: PasswordDashboardProps) {
  ...
  return (
    <div className="w-full">
      <div className="mb-6 flex items-center justify-between gap-4">
        <AddPasswordDialog onAddCredential={addCredentialToState} rawMasterPassword={rawMasterPassword} />
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {credentials.map(cred => (
            <PasswordCard key={cred.id} credential={cred} onDelete={deleteCredential} rawMasterPassword={rawMasterPassword} />
          ))}
        </div>
      ) : (
